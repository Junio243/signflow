...
          S                     : 'bg-slate-200 text-slate-600 hover:bg-slate-300'
                            }`}
...
                  S           checked={profileId === profile.id}
...
OS                       <span className="text-slate-500"> — {selectedProfile.theme.reg}</span>
                          )}
                        </div>
                      )}
...
CSS                   {selectedProfile.theme.footer}
                        </div>
                      )}
...
              <p className="text-sm text-slate-500">
  s             Conferiu tudo? Clique para gerar o PDF assinado e obter o link de validação com QR Code.
              </p>
              <Button className="mt-4 w-full" onClick={assinarESalvar} disabled={disableAction}>
s               {busy ? 'Processando…' : 'Assinar & gerar documento'}
              </Button>
              <p className="mt-2 text-xs text-slate-500">
section             O arquivo é processado em segundos. A prévia acima continuará disponível para ajustes.
              </p>
            </div>
          </section>
        </div>
...
section         <h2 className="text-lg font-semibold text-emerald-900">Documento pronto!</h2>
            <ul className="mt-3 space-y-2">
              {result.signed_pdf_url && (
                <li>
                  <span className="font-medium">PDF Assinado:</span>{' '}
S                 <a className="underline" href={result.signed_pdf_url} target="_blank" rel="noreferrer">
                    Abrir PDF
                  </a>
                </li>
              )}
