...
export default function SettingsPage() {
  ...
  async function fetchProfile(uid: string) {
    ...
    if (data) {
      ...
      setPhone(data.phone || '');
      setCity(data.city || 'Brasília/DF');
      setTimezone(data.timezone || 'America/Sao_Paulo');
      setLanguage(data.language || 'pt-BR');
      setPreferredSignatureId(data.preferred_signature_id || null);
    }
  }

  // fetch signatures for user
  async function fetchSignatures(uid: string) {
    ...
    if (error) {
      console.warn('signatures fetch error', error);
      return;
    }
  ...
  async function handleSaveProfile() {
    if (!sessionUserId) return setInfo('Não autenticado.');
    ...
    try {
      const payload = {
        ...
        timezone,
        language,
        preferred_signature_id: preferredSignatureId
      };
      const { error } = await supabase
        .from('profiles')
        .upsert(payload, { returning: 'minimal' }); // avoid double select
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    if (!sessionUserId) return setInfo('Faça login primeiro.');
  ...
  async function uploadSignatureFile(file: File, name = 'assinatura', type: 'draw'|'upload'|'certified' = 'upload') {
    if (!sessionUserId) return setInfo('Faça login primeiro.');
    setBusy(true); setInfo(null);
    try {
      ...
      // if first signature, make default
  ...
  async function setDefaultSignature(sigId: string) {
    ...
    try {
      ...
      if (unsetErr) {
        // fallback: update via query if rpc not present
        await supabase.from('signatures').update({ is_default: false }).eq('user_id', sessionUserId);
      }
      // set this one
      const { error: setErr } = await supabase.from('signatures').update({ is_default: true }).eq('id', sigId);
      if (setErr) throw setErr;

      // update profile preferred_signature_id
      const { error: profErr } = await supabase.from('profiles').upsert({
        id: sessionUserId,
        preferred_signature_id: sigId
      }, { returning: 'minimal' });
  ...
  // ---- DRAWING: simple canvas-based drawing ----
  useEffect(() => {
  ...
  const canvasCtx = () => signCanvasRef.current!.getContext('2d')!;
  ...
  async function saveCanvasSignature(name = 'Desenho') {
    ...
    try {
      ...
      console.error(e);
      setInfo('Erro ao salvar assinatura desenhada: ' + (e?.message ?? 'desconhecido'));
    } finally {
      setBusy(false);
    }
  }

  // helper: create a simple UI for anonymous/no-session
  ...
  if (!sessionUserId) {
    return (
      <div style={{ maxWidth: 900, margin: '20px auto', padding: 16 }}>
        ...
        <button onClick={() => router.push('/login')} style={{ padding: '8px 12px', borderRadius: 8 }}>Entrar</button>
      </div>
    );
  }
        ...
        <div style={{ display:'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div>
            ...
            <div style={{ marginTop: 8, display:'flex', gap:8 }}>
              <button onClick={() => saveCanvasSignature('Desenho')} disabled={busy} style={{ padding:'8px 12px', borderRadius:8, background:'#2563eb', color:'#fff' }}>Salvar desenho</button>
              <button onClick={clearCanvas} disabled={busy} style={{ padding:'8px 12px', borderRadius:8, background:'#fff', border:'1px solid #e5e7eb' }}>Limpar</button>
            </div>
          </div>
            ...
            {signatures.map(sig => (
              <div key={sig.id} style={{ display:'flex', gap:12, alignItems:'center', padding:8, border:'1px solid #eaeaea', borderRadius:8 }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:13, color:'#6b7280' }}>{new Date(sig.created_at).toLocaleString()}</div>
                </div>
